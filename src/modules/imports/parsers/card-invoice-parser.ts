...
function buildInvoicePrompt(): string {
  return [
    ...
    '- `categoryName`: sua MELHOR sugestão de categoria pra esse lançamento, a partir do nome do estabelecimento/descrição (ex.: "AZUL SEGUROS" → "Seguros"; "IOF TRANSACOES" → "Impostos"; "KaBuM" → "Informática"; STREAMING de vídeo/música (NETFLIX, SPOTIFY, DISNEY, PRIME VIDEO, HBO/MAX, YOUTUBE PREMIUM, DEEZER, GLOBOPLAY) → "Streaming"; assinatura de software/serviço (ANTHROPIC, OPENAI/CHATGPT, GITHUB, ICLOUD, GOOGLE ONE, MICROSOFT) → "Assinaturas"; "iFood"/restaurante → "Alimentação"; posto de combustível → "Transporte"). Streaming e Assinaturas são categorias SEPARADAS — não jogue streaming em Assinaturas. Categoria genérica em pt-BR, NUNCA o nome exato de uma categoria que você não viu na fatura. Sem certeza nenhuma → `null`, nunca force um palpite ruim.',
    "",
