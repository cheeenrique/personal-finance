...
type CardDetailFacePanelProps = {
  ...
  onEdit: () => void;
  onDelete: () => void;
  /** Motivo do bloqueio de "Editar cartão" — só o CREDIT detail view passa isso (CANCELLED). `undefined` = edição liberada (ACTIVE/BLOCKED, e sempre no MEAL). */
 ...
 */
export function CardDetailFacePanel({ card, onEdit, onDelete, editDisabledReason }: CardDetailFacePanelProps) {
  ...
  return (
    <div className="mx-auto flex w-full max-w-[380px] flex-col gap-3 lg:mx-0">
      ...
      <div className="flex gap-2.5">
        {editDisabledReason ? (
          <Tooltip>
            <TooltipTrigger render={<Button type="button" size="lg" disabled className="flex-1 gap-1.5" />}>
