import {
  LayoutDashboard,
  ArrowLeftRight,
  Wallet,
  CreditCard,
  Layers3,
  PiggyBank,
  Landmark,
  FolderTree,
  Tag,
  BarChart3,
  Bell,
  Settings,
  type LucideIcon,
} from "lucide-react";

/**
 * Fonte única de verdade da navegação — reusada por Sidebar, Header (mapa de
 * título/descrição por rota) e BottomNav/Drawer mobile (docs/06-SCREENS.md,
 * "Header" e "Sidebar": "mantido num só lugar, não hardcoded em cada tela").
 */
export type NavItem = {
  label: string;
  href: string;
  icon: LucideIcon;
  description: string;
};

export const NAV_ITEMS: NavItem[] = [
  {
    label: "Dashboard",
    href: "/dashboard",
    icon: LayoutDashboard,
    description: "Sua vida financeira agora",
  },
  {
    label: "Transações",
    href: "/transactions",
    icon: ArrowLeftRight,
    description: "Todas as suas movimentações financeiras",
  },
  {
    label: "Contas",
    href: "/accounts",
    icon: Wallet,
    description: "Saldo disponível e transferências entre contas",
  },
  {
    label: "Cartões",
    href: "/cards",
    icon: CreditCard,
    description: "Limite, fatura e compras dos seus cartões",
  },
  {
    label: "Parcelamentos",
    href: "/installments",
    icon: Layers3,
    description: "Compras parceladas em progresso",
  },
  {
    label: "Orçamentos",
    href: "/budgets",
    icon: PiggyBank,
    description: "Planejado vs. realizado por categoria",
  },
  {
    label: "Patrimônio",
    href: "/assets",
    icon: Landmark,
    description: "Bens e investimentos ao longo do tempo",
  },
  {
    label: "Categorias",
    href: "/categories",
    icon: FolderTree,
    description: "Estrutura de categorias usada nas transações",
  },
  {
    label: "Tags",
    href: "/tags",
    icon: Tag,
    description: "Marcadores livres para contextualizar transações",
  },
  {
    label: "Relatórios",
    href: "/reports",
    icon: BarChart3,
    description: "Análise profunda e comparação de tendências",
  },
  {
    label: "Alertas",
    href: "/alerts",
    icon: Bell,
    description: "Histórico completo de alertas gerados",
  },
  {
    label: "Configurações",
    href: "/settings",
    icon: Settings,
    description: "Preferências, alertas, Telegram e dados",
  },
];

/** Itens visíveis na Bottom Navigation (mobile) — os 3 mais usados + [+] + Menu. */
export const BOTTOM_NAV_ITEMS: NavItem[] = [
  NAV_ITEMS[0], // Dashboard
  NAV_ITEMS[1], // Transações
  NAV_ITEMS[3], // Cartões
];

/** Restante da navegação, exibido no Drawer "Menu" do mobile (mesma lista da Sidebar, exceto os já fixados na Bottom Nav). */
export const DRAWER_NAV_ITEMS: NavItem[] = NAV_ITEMS.filter(
  (item) => !BOTTOM_NAV_ITEMS.includes(item),
);

export function findNavItemByPathname(pathname: string): NavItem | undefined {
  return NAV_ITEMS.find(
    (item) => pathname === item.href || pathname.startsWith(`${item.href}/`),
  );
}
