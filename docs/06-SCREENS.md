...
- `pagination`: `/transactions` e o histórico de transações do detalhe de conta (`/accounts/[id]`, ver `21-ACCOUNTS.md`) paginam server-side. Parcelamentos (`/installments`) pagina client-side (grid `lg:grid-cols-3`, 9 por página, `DataTablePagination` com `?page=`). As demais telas (Contas — grid de cards, Cartões, Categorias, Tags, Orçamentos, Assets) usam a mesma `DataTable` com `pagination={false}` — carregam tudo de uma vez.
